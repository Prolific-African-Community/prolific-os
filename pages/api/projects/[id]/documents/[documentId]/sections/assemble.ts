...
export default withAuth(
  async (req: AuthenticatedNextApiRequest, res: NextApiResponse) => {
    ...
    try {
      ...
      const result = await assembleDocument(owned.document);
      if ("error" in result) {
      ...
      return res.status(200).json({
        success: true,
        message: `Assembled ${result.sectionsIncluded} sections (${result.totalWords} words).`,
        data: result,
      });
    } catch (error) {
      console.error("ASSEMBLE DOCUMENT ERROR:", error);
      return res
        .status(500)
        .json({ success: false, message: "Unable to assemble document" });
    }
  }
);
